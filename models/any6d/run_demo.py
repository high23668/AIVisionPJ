...
if __name__=='__main__':
    ...
    parser.add_argument("--img_to_3d", action="store_true", help="Running with SF3D+SAM2 (model-free, no CAD mesh required)")
    parser.add_argument("--mesh", type=str, default=None, help="Path to pre-generated mesh OBJ (skips img_to_3d step)")
    ...
    results = []
    demo_path = 'demo_data'
    mesh_path = args.mesh if args.mesh else os.path.join(demo_path, f'mustard.obj')
    ...
    if img_to_3d:
        ...
        sf3d_process(input_image, save_path, obj)
    ...
    print(f"Pose saved to {save_path}/{obj}_initial_pose.txt")

    # GT評価 (YCBデータセットが必要。なければスキップ)
    try:
