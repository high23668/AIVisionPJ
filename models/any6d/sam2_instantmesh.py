...
# SF3D paths (relative to this file)
_ANY6D_DIR = Path(__file__).parent
_SF3D_DEMO = _ANY6D_DIR.parent / "image_to_3d" / "sf3d_demo.py"
_VENV_PYTHON = _ANY6D_DIR.parent.parent / ".venv_3d" / "bin" / "python"
...
def sf3d_process(input_image, debug_dir, name=None):
    """
    SF3D (Stable Fast 3D) で単一RGBA画像からOBJメッシュを生成する。
    Zero123Plus + InstantMesh の2段パイプラインを置き換える。

    内部では .venv_3d の sf3d_demo.py をサブプロセスで呼び出し、
    出力 GLB を trimesh で OBJ に変換して mesh_{name}.obj として保存する。

    Args:
        input_image: PIL RGBA Image (preprocess_image() の出力)
        debug_dir:   出力ディレクトリ
        name:        オブジェクト名 (出力ファイル名に使用)
    """
    # 前処理済み画像を一時PNGとして保存
    tmp_png = os.path.join(debug_dir, f'sf3d_input_{name}.png')
    input_image.save(tmp_png)

    # 出力GLBパス
    glb_path = os.path.join(debug_dir, f'sf3d_{name}.glb')

    # SF3Dをサブプロセスで実行 (.venv_3d 環境)
    print(f"Running SF3D via subprocess: {_VENV_PYTHON}")
    result = subprocess.run(
        [str(_VENV_PYTHON), str(_SF3D_DEMO), "--image", tmp_png, "--output", glb_path],
        check=True,
    )

    # GLB → OBJ 変換 (trimesh: Scene の場合は dump() + concatenate)
    obj_path = os.path.join(debug_dir, f'mesh_{name}.obj')
