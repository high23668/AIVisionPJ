...
class MVSGSNetEval():
    ...
    def infer(self, rgb_path, ir1_path, ir2_path, mask_path=None, zrot=None):
        with torch.no_grad():
            ...
            # Save point cloud and grasp geometry (headless friendly)
            ...
            out_dir = os.environ.get("ASGRASP_OUT", "/results_out")
            os.makedirs(out_dir, exist_ok=True)
            o3d.io.write_point_cloud(os.path.join(out_dir, "scene_point_cloud.ply"), point_cloud)
            # Combine top grippers as a single mesh and save
            if len(grippers) > 0:
                combined = grippers[0]
                for g in grippers[1:]:
                    combined += g
                o3d.io.write_triangle_mesh(os.path.join(out_dir, "grasps_top.ply"), combined)

            # Top-N grasp poses to text
            top_n = min(len(gg), 10)
            with open(os.path.join(out_dir, "grasp_poses.txt"), "w") as f:
                f.write("# rank, score, width, depth, height, tx, ty, tz, R(3x3 row-major)\n")
                for i in range(top_n):
                    g = gg[i]
                    R = g.rotation_matrix.flatten()
                    t = g.translation
                    f.write(f"{i}, {g.score:.4f}, {g.width:.4f}, {g.depth:.4f}, {g.height:.4f}, "
                            f"{t[0]:.4f}, {t[1]:.4f}, {t[2]:.4f}, "
                            + ",".join(f"{v:.4f}" for v in R) + "\n")
            print(f"  Saved {top_n} grasps + point cloud to {out_dir}")

            # Try offscreen render (Open3D 0.15+ has OffscreenRenderer)
            if DEBUG_VIS:
                try:
                    import open3d.visualization.rendering as rendering
                    renderer = rendering.OffscreenRenderer(960, 720)
                    renderer.scene.set_background(np.array([0, 0, 0, 1]))
                    mat_pcd = rendering.MaterialRecord(); mat_pcd.shader = "defaultUnlit"
                    mat_grip = rendering.MaterialRecord(); mat_grip.shader = "defaultUnlit"; mat_grip.base_color = [0, 1, 0, 1]
                    renderer.scene.add_geometry("pcd", point_cloud, mat_pcd)
                    for i, g in enumerate(grippers[:top_n]):
                        renderer.scene.add_geometry(f"g{i}", g, mat_grip)
                    bbox = point_cloud.get_axis_aligned_bounding_box()
                    center = bbox.get_center(); extent = bbox.get_extent().max()
                    renderer.setup_camera(60.0, center, center + np.array([0, -extent*1.5, -extent*0.3]),
                                          np.array([0, 0, -1]))
                    img = renderer.render_to_image()
                    o3d.io.write_image(os.path.join(out_dir, "grasps_render.png"), img)
                    print(f"  Saved offscreen render: {out_dir}/grasps_render.png")
                except Exception as e:
                    print(f"  Offscreen render failed (non-fatal): {e}")
