...
実験で用いたガラス板には複数の視覚的手がかりが存在する。

- **エッジ輝線**：板の縁が光を屈折・反射して明るい線として写る
- **鏡面反射**：板面が照明を鏡のように反射する（特に上部）
- **スタンド**：板を立てているスタンドは不透明で確実に写る

これらの手がかりをモデルが深度推定に活かせるかどうかで結果が分かれる。

---

**② DA2：透明物体を含む合成データでの学習（論文に根拠あり）**

DA2 論文（arxiv 2406.09414）は透明物体への対応を**中心的な強み**として明示している。

**学習パイプライン（論文記載）**

DA2 は 2 段階で学習する。

1. **第1段階（教師モデル）**：Hypersim・TartanAir など5種の合成データセット（約60万枚）で教師モデルを学習。合成データはレンダリングによって生成されるため、**ガラスや鏡面反射など現実センサーが計測できない物体の正確な深度ラベルを取得できる**と論文が明示している。
2. **第2段階（生徒モデル）**：教師モデルが約6,200万枚の実写画像に擬似ラベルを付与し、生徒モデルを学習。まず affine-invariant（相対的深度構造）で学習し、その後メトリック深度でファインチューニング。

**透明物体への効果（論文記載）**

論文は Transparent Surface Challenge ベンチマークで DA2 が 83.6%、先代 DA1 が 53.5% という結果を報告しており、透明物体への対応が設計上の明確な改善点であることが確認できる。

本実験でガラス板に RMSE 19.3mm を達成できた背景には、合成データによる透明物体の学習が貢献している可能性が高い。

**次のステップへの示唆**

DA2 の強みが透明物体を含む合成データ学習に起因するなら、透明物体（ガラス板・樹脂シート）を含む合成シーンを追加学習データとして用いることで、他の深度モデルの精度を改善できる可能性がある。

---

**③ MoGe-2：結果は良好だが理由は不明**

MoGe-2 は 3D ポイントマップ（各ピクセルの3D座標）を出力するモデルで、全4シーンで唯一安定した結果を得た。ただし論文（arxiv 2507.02546）に透明物体への対応に関する記述はなく、なぜ透明ワークに有効であったかの理由は現時点では説明できない。

なお、MoGe-2 は法線を直接出力するヘッドを持たない（ポイントマップから事後的に計算することは可能）。「法線推定による効果」を理由として挙げることは論文上の根拠がないため、本考察では触れない。

---

**④ Metric3D v2：透過の理由は実験的事実として記録**

Metric3D v2 はガラス板・樹脂シートともに精度が大きく劣り、板を透過して背景と同じ深度を割り当てた。論文（arxiv 2404.15506）にはこの挙動や透明物体への言及は一切ない。

この透過挙動の原因については、論文に根拠がある説明を見つけられなかったため、「実験で観察された事実」として記録するにとどめる。
...
| モデル | 根拠の種類 | 考察 |
|---|---|---|
| DA2 | 論文に明示あり | 透明物体を含む合成データでの学習が設計上の強みと論文が明記 |
| MoGe-2 | 実験結果のみ | 全シーン安定という結果は確認済み。なぜ有効かは不明 |
| Depth Pro | 実験結果のみ | ガラス板で RMSE 20.3mm。理由は未検証 |
| Metric3D v2 | 実験結果のみ | 透過挙動を確認。原因は論文上の根拠なし |
