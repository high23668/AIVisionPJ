...
「透明物体はどのモデルでも見えないのでは」という疑問は自然である。実際、**学習用の正解深度データ（LiDAR・ステレオカメラ・RealSense 等）はいずれも透明物体を正しく計測できない**。そのため全モデルが「透明物体の領域 = 背景の深度」を正解として学習しており、原理的には全モデルが透過を起こしうる。

それでも結果に差が出た理由として、以下が考えられる。

**① ガラス板は「完全に透明」ではない**

実際のガラス板には、エッジ輝線（縁の明るい線）・鏡面反射・スタンドなどの視覚的手がかりが存在する。「ガラス板の領域 = 背景」と学習済みであっても、これらの手がかりを捉えるアーキテクチャを持つモデルは、部分的に板の存在を検出できる可能性がある。

**② 法線推定の有無（MoGe-2）**

MoGe-2 は深度と**表面法線マップ**を同時に推定する。法線推定は面の向きの変化（エッジや反射の方向変化）を捉えることが目的であり、ガラスのエッジ部で生じる輝度・反射方向の変化を手がかりとして拾いやすい。これが全シーン対応という結果につながった可能性がある。

**③ メトリック精度へのコミット度（Metric3D が特に悪い理由）**

Metric3D v2 は「実寸距離を正確に当てる」ことを最優先に設計されており、正準カメラ空間への座標変換など強い正規化処理を行う。この強力な正規化がエッジ輝線や反射といった微妙な視覚的手がかりを削いでしまい、透明領域でも「外観が示す背景の距離」を確信を持って出力する方向に働いたと考えられる。他のモデルに比べて「外れ値を出さない」という強みが、透明物体では逆に足かせとなっている。

**④ 学習データの構成（DA2 Indoor の強み）**

DA2 の Indoor 特化版は、近距離の室内環境（ガラステーブル・窓・棚など透明物体が含まれやすい環境）で収集された擬似ラベルを多く含む可能性がある。また affine-invariant（相対的な深度構造）を重視する学習目的は、絶対値が外れていても「前にある・後ろにある」という相対的な深度構造を保持しやすく、ガラス板のエッジでの局所的な深度変化を捉えやすかったと考えられる。

**まとめると**、透明物体への対応力の差は「透過するかしないか」の二値ではなく、モデルが RGB 上の微細な手がかり（エッジ輝線・反射・スタンド形状）をどれだけ深度推定に活用できるかによって連続的に変化する。Metric3D v2 はメトリック精度特化の設計がこの活用を妨げた点で特徴的であり、他の 3 モデル（DA2・MoGe-2・Depth Pro）は異なる理由でそれぞれ部分的に板の存在を検出できたと解釈できる。
