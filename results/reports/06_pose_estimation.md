...
Z 誤差は FoundationPose が出力する 4×4 変換行列の並進成分（translation）の Z 値と、青紙で計測した GT Z の差である。

```
出力行列（camera → object）の右列:
  t = [x, y, z]  ← z が「カメラから CAD 原点までの Z 方向距離（m）」
```

実際の出力値（DA2, glass_front）:
- FP 出力 Z = 383.72mm
- GT Z = 383.6mm
- 差 = **+0.12mm ≈ +0.1mm**
...
CAD モデルの原点が板の中心（厚み方向の中央）に設定されている場合、FP 出力 Z は板の前面ではなく中心までの距離を指す。板厚が 2.5mm なので最大 1.25mm のオフセットが生じうる。青紙 GT Z は板の**表面**を計測した値であるため、両者の定義が厳密には一致していない点に注意が必要である。

**Affine 校正との循環性について**

本実験の Affine 校正は「青紙で計測した GT Z を校正の基準点の一つとして使用する」設計であり、校正後の深度は板領域の平均値が GT Z に近くなるよう調整されている。そのため、FP 出力 Z が GT Z に近い値になることは**校正設計上ある程度 built-in されている**。

この結果が示すことは「Affine 校正済みの深度（RMSE ≈19mm）を入力すれば、FP × CAD はそれをさらに高精度なポーズに変換できる」であり、「GT Z 計測なし・青紙なしの実運用でも 0.1mm が出る」を意味するものではない。壁校正のみ（GT Z 不要）を用いた場合の精度は未計測であり、今後の検証課題である。
