"""Phase 7.2 MoGe-2 (Microsoft) ガラス板 depth 推定 + RS壁面で scale 校正 + FP scene 出力."""
import argparse
import json
import math
import shutil
...
def load_moge(repo: str = "Ruicheng/moge-2-vitl-normal"):
    from moge.model.v2 import MoGeModel
    model = MoGeModel.from_pretrained(repo)
    model.cuda().eval()
    return model


def predict_depth(model, rgb_bgr: np.ndarray, fx: float):
    rgb = rgb_bgr[:, :, ::-1].copy()
    h, w = rgb.shape[:2]
    ...
    mask = out.get("mask")
    if mask is not None:
        depth = np.where(mask.squeeze().cpu().numpy().astype(bool), depth, np.nan)
...
def visualize(rgb_bgr, depth_raw, depth_cal, rs_depth, plate_mask, out_path: Path, model_name: str, scene: str):
    ...
    fig.suptitle(f"Phase 7.2 {model_name} ({scene})", fontsize=14)
    fig.tight_layout()
...
def run_scene(model, scene: str, model_tag: str, out_results: Path):
    ...
    depth_raw = predict_depth(model, rgb, fx)
...
def main():
    ...
    ap.add_argument("--repo", default="Ruicheng/moge-2-vitl-normal")
    ap.add_argument("--tag", default="moge2")
    args = ap.parse_args()

    out_results = ROOT / "results" / "glassboard"
    out_results.mkdir(parents=True, exist_ok=True)

    print(f"[Phase 7.2] Loading {args.repo} ...")
    model = load_moge(args.repo)
    summaries = []
    for scene in args.scenes:
        print(f"[Phase 7.2] Running scene={scene} ...")
